...
║   Expiry optimisé par instrument       ║
║   V10(30min) V25(15min) V75(1min)      ║
║   Trades simultanés corrigés           ║
...
CONFIG = {
    # === API DERIV ===
    "deriv_token"   : "VOTRE_TOKEN_ICI",
    "deriv_app_id"  : "VOTRE_APP_ID_ICI",

    # === TELEGRAM ===
    "telegram_token"   : "VOTRE_TELEGRAM_TOKEN_ICI",
    "telegram_chat_id" : "VOTRE_CHAT_ID_ICI",
    "telegram_enabled" : True,

    # === INSTRUMENTS (expiry optimisé par instrument) ===
    "instruments": {
        "R_10": {"expiry": 30, "name": "Volatility 10"},
        "R_25": {"expiry": 15, "name": "Volatility 25"},
        "R_75": {"expiry": 1,  "name": "Volatility 75"},
    },
...
# ============================================================
#
#                  DÉTECTION ZONES S/R (ZIGZAG M15)
#
# ============================================================
...
class TradingBot:
    ...
    def __init__(self):
        ...
        # v2.1 : Gestion trades simultanés
        self.pending_trades = {}
        self._req_id = 0
        self.open_trades = {}
    ...
    def run(self):
        ...
        log.info("📋 Trades simultanés activés")
        ...
        log.info(f"📐 Doji: {'ON' if CONFIG['use_doji'] else 'OFF'}")
        ...
        telegram(
            ...
            f"📐 Doji: {'ON' if CONFIG['use_doji'] else 'OFF'}\n"
    ...
    def _hist(self, data):
        ...
        if gran == 900:
            ...
            log.info(f"📍 {info['name']} | "
                     f"{len(self.zones[sym])} zones ({act} actives)")
        else:
            self.m1_ok[sym] = True
            log.info(f"📊 {info['name']} | "
                     f"M1 prêt ({len(self.m1[sym])} bougies)")
    ...
    def _ohlc(self, data):
        ...
        if gran == 900:
            ...
            if buf and candle.time != buf[-1].time:
                ...
                info = CONFIG["instruments"][sym]
                act = sum(1 for z in self.zones[sym] if z.broken_time == 0)
                log.info(f"🔄 {info['name']} | Zones: {act} actives")
            elif buf:
                buf[-1] = candle
    ...
    def _check_signal(self, sym):
        ...
        if datetime.now().day != self.last_day:
            ...
            self.daily_trades = 0
            self.last_day = datetime.now().day
            log.info("🔄 Nouveau jour - compteurs réinitialisés")
            # Envoyer stats de la veille
            telegram(
                "🔄 <b>Nouveau jour</b>\n"
        ...
        # Cooldown par instrument
        now = time.time()
        if now - self.last_sig.get(sym, 0) < CONFIG["cooldown"] * 60:
            return
        ...
        log.info(f"🎯 {ctype} {info['name']} | {pattern} | "
                 f"Expiry: {info['expiry']}min")

        self._trade(sym, ctype, current.close, pattern)

    # =============================================
    #    v2.1 : TRADE avec req_id unique
    # =============================================
    ...
    def _trade(self, sym, ctype, price, pattern):
        ...
        trade = Trade(time.time(), ctype, sym, price,
                      CONFIG["stake"], expiry)
    ...
    # =============================================
    #    v2.1 : ACHAT avec req_id matching
    # =============================================
    ...
    def _bought(self, data):
        ...
        if req_id in self.pending_trades and cid:
            ...
            log.info(f"📝 Trade ouvert | {info['name']} | "
                     f"{trade.direction} | ID: {cid}")
    ...
    def _contract(self, data):
        ...
        # Log
        if trade.is_win:
            log.info(f"✅ WIN +{profit:.2f}$ | "
                     f"{info['name']} {trade.direction} "
                     f"({trade.expiry}min)")
        else:
            log.info(f"❌ LOSS {profit:.2f}$ | "
                     f"{info['name']} {trade.direction} "
                     f"({trade.expiry}min)")

        # Stats
        day = self.stats.today()
        day_sym = self.stats.today(trade.symbol)
        sk = (f"{day['streak']}W" if day['streak'] > 0
              else f"{abs(day['streak'])}L" if day['streak'] < 0
              else "0")
        ...
        telegram(
            f"{emoji} <b>{'WIN' if trade.is_win else 'LOSS'} "
            f"{pstr}$</b>\n"
            f"{info['name']} | {trade.direction} | "
            f"{trade.expiry}min\n"
            f"Série: {sk} | WR jour: {day['winrate']:.1f}%\n"
            f"Profit jour: {day['profit']:.2f}$ "
            f"({day['total']} trades)\n"
            f"Trades actifs: {len(self.open_trades) - 1}"
        )

        # Stats complètes tous les 10 trades
